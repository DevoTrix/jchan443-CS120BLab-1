...
// learned from zybooks
int getBit(int num, int bit){
  return ( num & ( 0x01 << bit)) != 0;
}

void shiftTilEnd(int pos){
  if(pos == 0){
    return;
  }
  int data = getHex();
  data >> 1;
  digitalWrite(B[pos], LOW);
  digitalWrite(B[pos-1], HIGH);
  delay(100);
  return shiftTilEnd(pos-1);
}
/*
...
void loop() {
  ...
  */
  ...
  reading = reading & (0x01 << access);
  delay(100);
  if(getBit(reading, access) != 0){
    digitalWrite(B[access], HIGH);
    delay(100);
  	shiftTilEnd(access);
  }
